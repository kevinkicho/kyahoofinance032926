...
export const PANEL_REGISTRY = {
  ...
  fx: [
    { id: 'sidebar', title: 'FX Dashboard', field: 'spotRates', fieldPath: 'spotRates', source: 'fx.js', external: [{ name: 'Frankfurter / FRED', seriesIds: ['DEXUSEU','DEXJPUS'] }], renderCheck: 'spotRates && Object.keys(spotRates).length > 0' },
    { id: 'movers', title: 'Top Movers', field: 'changes1d', fieldPath: 'changes1d', source: 'fx.js', external: [{ name: 'Frankfurter', seriesIds: [] }], renderCheck: 'changes1d && Object.keys(changes1d).length > 0' },
  ...
  crypto: [
    { id: 'top-cryptos', title: 'Top Cryptos', field: 'coinMarketData', fieldPath: 'coinMarketData.coins', source: 'crypto.js', external: [{ name: 'CoinGecko', seriesIds: [] }], renderCheck: 'coinMarketData?.coins?.length > 0' },
    ...
    { id: 'defi-tvl', title: 'DeFi TVL by Chain', field: 'defiData', fieldPath: 'defiData', source: 'crypto.js', external: [{ name: 'DefiLlama', seriesIds: [] }], renderCheck: 'defiData && Object.keys(defiData).length > 0' },
    { id: 'funding', title: 'Funding Rates', field: 'fundingData', fieldPath: 'fundingData', source: 'crypto.js', external: [{ name: 'Bybit', seriesIds: [] }], renderCheck: 'fundingData && Object.keys(fundingData).length > 0' },
  ...
  derivatives: [
    ...
    { id: 'vixterm', title: 'VIX Term Structure', field: 'vixTermStructure', fieldPath: 'vixTermStructure', source: 'derivatives.js', external: [{ name: 'Yahoo Finance / FRED', seriesIds: ['VIXCLS'] }], renderCheck: 'vixTermStructure && vixTermStructure.length > 0' },
    ...
    { id: 'volsurf', title: 'Vol Surface', field: 'volSurfaceData', fieldPath: 'volSurfaceData', source: 'derivatives.js', external: [{ name: 'Computed', seriesIds: [] }], renderCheck: 'volSurfaceData && Object.keys(volSurfaceData).length > 0' },
    { id: 'flow', title: 'Options Flow', field: 'optionsFlow', fieldPath: 'optionsFlow', source: 'derivatives.js', external: [{ name: 'Computed', seriesIds: [] }], renderCheck: 'optionsFlow && Object.keys(optionsFlow).length > 0' },
  ...
  realEstate: [
    { id: 'shiller', title: 'Case-Shiller Index', field: 'caseShillerData', fieldPath: 'caseShillerData', source: 'realEstate.js', external: [{ name: 'FRED', seriesIds: ['CSUSHPISA'] }], renderCheck: 'caseShillerData && (caseShillerData.national || caseShillerData.values)', renderType: 'SafeECharts' },
    { id: 'reitperf', title: 'REIT Performance', field: 'reitData', fieldPath: 'reitData', source: 'realEstate.js', external: [{ name: 'Yahoo Finance', seriesIds: ['VNQ','O','SPG'] }], renderCheck: 'reitData && reitData.length > 0' },
    { id: 'afford-stack', title: 'Housing Affordability Stack', field: 'affordabilityData', fieldPath: 'affordabilityData', source: 'realEstate.js', external: [{ name: 'FRED / HUD / Census', seriesIds: ['MEHOINUSA672N'] }], renderCheck: 'affordabilityData && (affordabilityData.current || Object.keys(affordabilityData).length > 0)' },
    { id: 'caprate', title: 'Cap Rates by Sector', field: 'capRateData', fieldPath: 'capRateData', source: 'realEstate.js', external: [{ name: 'FRED', seriesIds: ['MORTGAGE30US','DGS10'] }], renderCheck: 'Array.isArray(capRateData) && capRateData.length > 0' },
  ...
  insurance: [
    { id: 'catbonds', title: 'Cat Bond Spreads', field: 'catBondSpreads', fieldPath: 'catBondSpreads', source: 'insurance.js', external: [{ name: 'Computed', seriesIds: [] }], renderCheck: 'catBondSpreads && catBondSpreads.length > 0' },
    ...
    { id: 'reinsrates', title: 'Reinsurance Pricing', field: 'reinsurancePricing', fieldPath: 'reinsurancePricing', source: 'insurance.js', external: [{ name: 'Computed', seriesIds: [] }], renderCheck: 'reinsurancePricing && (reinsurancePricing.byCategory || reinsurancePricing.length > 0)' },
    { id: 'reserves', title: 'Reserve Adequacy', field: 'reserveAdequacyData', fieldPath: 'reserveAdequacyData', source: 'insurance.js', external: [{ name: 'FRED / SEC EDGAR', seriesIds: [] }], renderCheck: 'reserveAdequacyData && Object.keys(reserveAdequacyData).length > 0' },
  ...
  commodities: [
    { id: 'prices', title: 'Commodity Prices', field: 'priceDashboardData', fieldPath: 'priceDashboardData', source: 'commoditiesEnhanced.js:298 (EIA+Yahoo)', external: [{ name: 'FRED / EIA / Yahoo', seriesIds: ['GOLDAMGBD228NLBM','POILWTIUSDM'] }], renderCheck: 'priceDashboardData && priceDashboardData.length > 0' },
    { id: 'futures', title: 'Futures Curve', field: 'futuresCurveData', fieldPath: 'futuresCurveData', source: 'commoditiesEnhanced.js:498 (Yahoo CME)', external: [{ name: 'Yahoo Finance', seriesIds: [] }], renderCheck: 'futuresCurveData && futuresCurveData.labels?.length > 0', renderType: 'FuturesCurve component' },
    ...
    { id: 'supply', title: 'Supply & Demand', field: 'supplyDemandData', fieldPath: 'supplyDemandData', source: 'commoditiesEnhanced.js:324 (EIA)', external: [{ name: 'EIA / USDA', seriesIds: [] }], renderCheck: 'supplyDemandData && Object.keys(supplyDemandData).length > 0' },
    ...
    { id: 'us-trade', title: 'US Trade Balance', field: 'blocs', fieldPath: 'blocs', crossMarket: 'censusTrade', source: 'censusTrade.js', external: [{ name: 'US Census Bureau', seriesIds: [] }], renderCheck: 'tradeCtx?.data?.blocs?.length > 0', renderType: 'SafeECharts' },
