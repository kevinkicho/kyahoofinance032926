    ...
    <div className="eq-panel">
      ...
      {kpis && (
        <div className="eq-kpi-strip">
          <div className="eq-kpi-pill">
            ...
            <span className="eq-kpi-sub">{kpis.topFactor.val >= 0 ? '+' : ''}{kpis.topFactor.val.toFixed(1)}pp{kpis.topFactorStock ? ` · Top ${kpis.topFactorStock.ticker}${kpis.topFactorStock.name ? ` (${kpis.topFactorStock.name})` : ''} ${kpis.topFactorStock[kpis.topFactor.key]?.toFixed(0)}` : ''}</span>
          </div>
          <div className="eq-kpi-pill">
            <span className="eq-kpi-label">Top Stock</span>
            <span className="eq-kpi-value accent">{kpis.topStock.ticker}</span>
            <span className="eq-kpi-sub">{kpis.topStock.name ? `${kpis.topStock.name} · ` : ''}{kpis.topStock.sector ? `${kpis.topStock.sector} · ` : ''}Composite {kpis.topStock.composite}{(() => { const s = kpis.topStock; const factors = [{ name: 'Value', val: s.value }, { name: 'Momentum', val: s.momentum }, { name: 'Quality', val: s.quality }, { name: 'Low-Vol', val: s.lowVol }].filter(f => f.val != null); if (!factors.length) return ''; const best = factors.reduce((a, b) => (b.val ?? 0) > (a.val ?? 0) ? b : a); const worst = factors.reduce((a, b) => (b.val ?? 999) < (a.val ?? 999) ? b : a); return ` · ${best.name} ${best.val.toFixed(0)} · ${worst.name} ${worst.val.toFixed(0)}`; })()}</span>
          </div>
          <div className="eq-kpi-pill">
            <span className="eq-kpi-label">Avg Composite</span>
            <span className="eq-kpi-value accent">{kpis.avgComposite.toFixed(0)}</span>
            <span className="eq-kpi-sub">{kpis.topStock ? `Top: ${kpis.topStock.ticker}${kpis.topStock.name ? ` (${kpis.topStock.name})` : ''} (${kpis.topStock.composite})${kpis.topStock.composite != null && kpis.avgComposite != null ? ` · +${Math.round(kpis.topStock.composite - kpis.avgComposite)} above avg` : ''}` : ''}</span>
          </div>
          <div className="eq-kpi-pill">
            <span className="eq-kpi-label">{`Quality \u2265 70`}</span>
            <span className="eq-kpi-value accent">{kpis.highQuality}</span>
            <span className="eq-kpi-sub">of {stocks.length}{stocks.length > 0 ? ` · ${Math.round(kpis.highQuality / stocks.length * 100)}%` : ''}{kpis.topQualityStock ? ` · Top: ${kpis.topQualityStock.ticker}${kpis.topQualityStock.name ? ` (${kpis.topQualityStock.name})` : ''} ${kpis.topQualityStock.quality?.toFixed(0)}` : ''}</span>
          </div>
        </div>
      )}
      {/* Breadth Divergence */}
      ...
      <div className="eq-two-row">
        <div className="eq-chart-panel">
          ...
          <div className="eq-chart-subtitle">Percentile deviation from 50 · indigo = positive · which factor is working</div>
