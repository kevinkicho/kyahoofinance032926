      ...
      {kpis && (
        <div className="eq-kpi-strip">
          <div className="eq-kpi-pill">
            ...
            <span className="eq-kpi-sub">{fmtValue(kpis.totalBuyValue)}{kpis.buyCount > 0 ? ` · avg ${fmtValue(kpis.totalBuyValue / kpis.buyCount)}` : ''}</span>
          </div>
          <div className="eq-kpi-pill">
        ...
        <div className={`eq-two-col ${!(txByTickerOption && holdersOption) ? 'eq-two-col--single' : ''}`}>
          {txByTickerOption && (
            <div className="eq-chart-panel">
              ...
              <div className="eq-chart-subtitle">Green = buys · Red = sells · Gray = other · top 10</div>
                ...
                {displayTx.map((t, i) => {
                  ...
                  return (
                    <tr key={`${t.ticker}-${t.date}-${t.name}-${i}`} className="eqd-row">
                      ...
                      {showTitleCol && <td className="eqd-cell eqd-name">{(() => { const rel = (t.relation || '').trim(); if (!role && !rel) return '\u2014'; if (rel && rel !== role) return `${role} (${rel})`; return role || rel || '\u2014'; })()}</td>}
