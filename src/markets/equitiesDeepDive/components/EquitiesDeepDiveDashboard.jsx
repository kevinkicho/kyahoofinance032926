      ...
      <>
          ...
          <div className="eqd-metric-card">
            ...
            {sectorKpis.best.perf1d != null ? (
              <div className="eqd-metric-row">
                ...
                <span className="eqd-metric-num eqd-name">1D {(sectorKpis.best.perf1d >= 0 ? '+' : '')}{sectorKpis.best.perf1d.toFixed(1)}% · 1M {(sectorKpis.best.perf1m ?? 0) >= 0 ? '+' : ''}{(sectorKpis.best.perf1m ?? 0).toFixed(1)}%{sectorKpis.best.perf3m != null ? ` · 3M ${sectorKpis.best.perf3m >= 0 ? '+' : ''}${sectorKpis.best.perf3m.toFixed(1)}%` : ''}</span>
              </div>
            ) : null}
            <div className="eqd-metric-row">
            ...
            {sectorKpis.worst.perf1d != null ? (
              <div className="eqd-metric-row">
                <span className="eqd-metric-name" />
                <span className="eqd-metric-num eqd-name">1D {(sectorKpis.worst.perf1d >= 0 ? '+' : '')}{sectorKpis.worst.perf1d.toFixed(1)}% · 1M {(sectorKpis.worst.perf1m ?? 0) >= 0 ? '+' : ''}{(sectorKpis.worst.perf1m ?? 0).toFixed(1)}%{sectorKpis.worst.perf3m != null ? ` · 3M ${sectorKpis.worst.perf3m >= 0 ? '+' : ''}${sectorKpis.worst.perf3m.toFixed(1)}%` : ''}</span>
              </div>
            ) : null}
            <div className="eqd-metric-row">
        ...
        {factorKpis && (
          <div className="eqd-metric-card">
            ...
            {factorKpis.topFactor.value != null ? (
              <div className="eqd-metric-row">
                <span className="eqd-metric-name" />
                <span className="eqd-metric-num eqd-name">{Number(factorKpis.topFactor.value).toFixed(1)}pp{factorKpis.topFactorStock ? ` · Top ${factorKpis.topFactorStock.ticker}${factorKpis.topFactorStock.name ? ` (${factorKpis.topFactorStock.name})` : ''} ${factorKpis.topFactorStock[factorKpis.topFactor.key]?.toFixed(0)}` : ''}</span>
              </div>
            ) : null}
            <div className="eqd-metric-row">
        ...
        {upcoming.length > 0 && (
          <div className="eqd-metric-card">
            ...
            {upcoming[0].epsEst != null && upcoming[0].epsPrev != null ? (
              <div className="eqd-metric-row">
                <span className="eqd-metric-name" />
                <span className="eqd-metric-num eqd-name">est ${Number(upcoming[0].epsEst).toFixed(2)} {Number(upcoming[0].epsEst) >= Number(upcoming[0].epsPrev) ? '▲' : '▼'} prior ${Number(upcoming[0].epsPrev).toFixed(2)}</span>
              </div>
            ) : null}
            <div className="eqd-metric-row">
    ...
    const earningsQualityBody = (
      <>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8, marginBottom: 10 }}>
          {[
            { label: 'Next Report', value: earningsQuality.next?.ticker || '—', sub: [earningsQuality.next?.name, earningsQuality.next?.sector, earningsQuality.next?.date, earningsQuality.next?.marketCapB != null ? `$${earningsQuality.next.marketCapB}B` : null, (earningsQuality.next?.epsEst != null && earningsQuality.next?.epsPrev != null ? `est $${earningsQuality.next.epsEst} ${Number(earningsQuality.next.epsEst) >= Number(earningsQuality.next.epsPrev) ? '▲' : '▼'} vs prior $${earningsQuality.next.epsPrev}` : null)].filter(Boolean).join(' · ') || 'schedule' },
            { label: 'Beat Breadth', value: earningsQuality.avgBeatRate != null ? `${earningsQuality.avgBeatRate.toFixed(1)}%` : '—', valueColor: earningsQuality.avgBeatRate != null ? (earningsQuality.avgBeatRate > 60 ? '#22c55e' : earningsQuality.avgBeatRate >= 50 ? '#f59e0b' : '#ef4444') : '#6366f1', sub: [earningsQuality.bestBeat?.sector ? `${earningsQuality.bestBeat.sector} ${earningsQuality.bestBeat.beatRate?.toFixed(0)}%` : null, earningsQuality.worstBeat?.sector ? `worst ${earningsQuality.worstBeat.sector} ${earningsQuality.worstBeat.beatRate?.toFixed(0)}%` : null].filter(Boolean).join(' · ') || (earningsQuality.bestBeat?.sector || 'by sector') },
            { label: 'Positive Revisions', value: earningsQuality.revisionRate != null ? `${earningsQuality.revisionRate.toFixed(0)}%` : '—', valueColor: earningsQuality.revisionRate != null ? (earningsQuality.revisionRate > 50 ? '#22c55e' : earningsQuality.revisionRate >= 40 ? '#f59e0b' : '#ef4444') : '#6366f1', sub: `${earningsQuality.positiveRevisions ?? 0}/${earningsQuality.totalUpcoming} upcoming` },
            { label: 'Top Factor', value: earningsQuality.topFactor?.name || '—', sub: [earningsQuality.topFactor?.value != null ? `${Number(earningsQuality.topFactor.value).toFixed(1)}pp` : null, earningsQuality.topFactorStock ? `Top ${earningsQuality.topFactorStock.ticker}${earningsQuality.topFactorStock.name ? ` (${earningsQuality.topFactorStock.name})` : ''} ${earningsQuality.topFactorStock[earningsQuality.topFactor.key]?.toFixed(0)}` : null].filter(Boolean).join(' · ') || 'rotation' },
          ].map(item => (
            <div key={item.label} className="eqd-metric-card" style={{ margin: 0 }}>
              <div className="eqd-sidebar-title">{item.label}</div>
              <div className="eqd-metric-row">
                <span className="eqd-metric-num" style={{ color: item.valueColor || '#6366f1', fontSize: 14 }}>{item.value}</span>
              </div>
        ...
        <table className="eqd-table">
          ...
          <tbody>
            <tr className="eqd-row">
              ...
              <td className="eqd-cell eqd-num">{earningsQuality.qualityCount}/{stocks.length || 0}{stocks.length > 0 ? ` · ${Math.round(earningsQuality.qualityCount / stocks.length * 100)}%` : ''}</td>
              <td className="eqd-cell">{(() => {
                const hq = stocks.filter(s => Number(s.quality ?? 0) >= 70).sort((a, b) => (b.composite ?? 0) - (a.composite ?? 0))[0];
                return hq ? `Top: ${hq.ticker}${hq.name ? ` (${hq.name})` : ''}${hq.sector ? ` · ${hq.sector}` : ''} (q${hq.quality ?? '?'}, c${hq.composite ?? '?'})` : 'Quality score 70+';
              })()}</td>
            </tr>
            <tr className="eqd-row">
              ...
              <td className="eqd-cell">{(() => {
                ...
                return top ? `Top: ${top.ticker}${top.name ? ` (${top.name})` : ''}${top.sector ? ` · ${top.sector}` : ''} (${top.composite})${top.composite != null && earningsQuality.avgComposite != null ? ` · +${Math.round(top.composite - earningsQuality.avgComposite)} above avg` : ''}` : 'Factor universe';
              })()}</td>
            </tr>
            <tr className="eqd-row">
