      ...
      <div className="eqd-mini-table">
        {institutions.slice(0, 6).map((inst, i) => (
          <div key={i} className="eqd-mini-row">
            ...
            {inst.topHoldings?.[0] ? <span className="eqd-mini-sub">Top: {inst.topHoldings[0].ticker} · {inst.topHoldings[0].pctOfPortfolio != null ? `${inst.topHoldings[0].pctOfPortfolio.toFixed(1)}%` : ''}</span> : null}
          </div>
        ))}
      </div>
    ) : null;
    ...
    const earningsQualityBody = (
      <>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 8, marginBottom: 10 }}>
          {[
            { label: 'Next Report', value: earningsQuality.next?.ticker || '—', sub: [earningsQuality.next?.name, earningsQuality.next?.date, (earningsQuality.next?.epsEst != null && earningsQuality.next?.epsPrev != null ? `est $${earningsQuality.next.epsEst} vs prior $${earningsQuality.next.epsPrev}` : null)].filter(Boolean).join(' · ') || 'schedule' },
