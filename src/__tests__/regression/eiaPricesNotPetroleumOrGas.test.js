/**
 * Regression: EIA electricity retail prices must not go green from
 * petroleum / naturalGas when electricity sector prices are empty.
...
describe('eia prices leftover petroleum/naturalGas wiring', () => {
  it('placeholders and field map bind electricity only', () => {
    const slots = slotPaths('eia', 'prices');
    expect(slots.some((p) => String(p).includes('electricity'))).toBe(true);
    expect(slots.some((p) => String(p).includes('petroleum') || String(p).includes('naturalGas'))).toBe(false);

    const spec = specPaths('eia', 'prices');
    expect(spec.includes('electricity')).toBe(true);
    expect(spec.includes('petroleum') || spec.includes('naturalGas')).toBe(false);
  });

  it('petroleum / naturalGas do not make prices L1 fetchOk', () => {
    const eia = {
  ...
  it('electricity sector prices still fill L1', () => {
    const eia = {
      data: {
        petroleum: PETROLEUM,
        naturalGas: NATGAS,
